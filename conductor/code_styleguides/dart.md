...
- **DO** use spread elements (`...`, `...?`) to compose collections (e.g., `{...list}` instead of `list.toSet()`).
...
- **DON'T** use `List.from()` unless you intend to change the type of the result; prefer collection spread elements `[...other]`.
